...
import { Plus, X, Mail, User, Shield, CheckCircle } from 'lucide-react'
import { addStaffMember } from '@/lib/firebase/hospital'
...
export function AddStaffDialog({ hospitalId }: { hospitalId: string }) {
  ...
  const [formData, setFormData] = useState({
    name: '',
    email: '',
    role: 'nurse' as 'admin' | 'doctor' | 'nurse' | 'receptionist'
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addStaffMember(hospitalId, {
        clerkUserId: `pending_${Date.now()}`, // This would be updated when they sign up
      ...
      setError(error instanceof Error ? error.message : 'Failed to add staff member. Please try again.')
    } finally {
      setLoading(false)
    }
  }
